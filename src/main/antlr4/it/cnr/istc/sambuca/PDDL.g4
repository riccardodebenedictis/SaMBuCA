...
domain:
	'(' 'define' '(' 'domain' name ')' require_def? types_def? constants_def? predicates_def?
		functions_def? constraints? structure_def* ')';
...
functions_def:
	{requirements.contains(":fluents") || requirements.contains(":numeric-fluents") || requirements.contains(":object-fluents")
		}? '(' ':functions' function_typed_list_atomic_function_skeleton? ')';
...
emptyOr_pre_GD: '(' ')' | pre_GD;

emptyOr_effect: '(' ')' | effect;

emptyOr_da_GD: '(' ')' | da_GD;

emptyOr_da_effect: '(' ')' | da_effect;
...
action_def_body: (':precondition' emptyOr_pre_GD)? (
...
pre_GD:
	...
	| {requirements.contains(":universal-preconditions")}? '(' 'forall' '(' typed_list_variable? ')'
		pre_GD ')' # pre_GD_forall;

pref_GD:
	{requirements.contains(":preferences")}? '(' 'preference' pref_name? gD ')' #
		pref_GD_preference_gD
	| gD # pref_GD_gD;
...
gD:
	...
	| {requirements.contains(":existential-preconditions")}? '(' 'exists' '(' typed_list_variable?
		')' gD ')' # gd_exists
	| {requirements.contains(":universal-preconditions")}? '(' 'forall' '(' typed_list_variable? ')'
		gD ')'												# gd_forall
...
name: NAME | 'at' | 'over'; // this is just terrible..
...
c_effect:
	{requirements.contains(":conditional-effects")}? '(' 'forall' '(' typed_list_variable? ')'
		effect ')'																		# c_effect_forall
...
p_effect:
	...
	| {requirements.contains(":numeric-fluents")}? '(' assign_op f_head f_exp ')'	#
		p_effect_assign_op_f_head_f_exp
	| {requirements.contains(":object-fluents")}? '(' 'assign' function_term term ')' #
		p_effect_assign_term
	| {requirements.contains(":object-fluents")}? '(' 'assign' function_term 'undefined' ')' #
		p_effect_assign_undefined;
...
pref_timed_GD:
	...
	| {requirements.contains(":preferences")}? '(' 'preference' pref_name? timed_GD ')'	#
		pref_timed_GD_preference_timed_GD;
...
duration_constraint:
	{requirements.contains(":duration-inequalities")}? '(' 'and' simple_duration_constraint+ ')' #
		duration_constraint_and
...
da_effect:
	...
	| {requirements.contains(":conditional-effects")}? '(' 'forall' '(' typed_list_variable? ')'
		da_effect ')'																		# da_effect_forall
	| {requirements.contains(":conditional-effects")}? '(' 'when' da_GD timed_effect ')'	#
		da_effect_when;

timed_effect:
	'(' 'at' time_specifier cond_effect ')'													# timed_effect_cond_effect
	| {requirements.contains(":numeric-fluents")}? '(' 'at' time_specifier f_assign_da ')'	#
		timed_effect_f_assign_da
	| {requirements.contains(":continuous-effects") && requirements.contains(":numeric-fluents")}?
		'(' assign_op_t f_head f_exp_t ')' # timed_effect_assign_op;
...
problem:
	'(' 'define' '(' 'problem' name ')' '(' ':domain' name ')' require_def? object_declaration? init
		goal constraints? metric_spec? length_spec? ')';
...
init_el:
	...
	| {requirements.contains(":timed-initial-literals")}? '(' 'at' NUMBER literal_name ')'	#
		init_el_at
	| {requirements.contains(":numeric-fluents")}? '(' '=' basic_function_term NUMBER ')' #
		init_el_eq_number
	| {requirements.contains(":object-fluents")}? '(' '=' basic_function_term name ')' #
		init_el_eq_name;
...
pref_con_GD:
	...
	| {requirements.contains(":universal-preconditions")}? '(' 'forall' '(' typed_list_variable? ')'
		pref_con_GD ')'																	# pref_con_GD_forall
	| {requirements.contains(":preferences")}? '(' 'preference' pref_name? con_GD ')'	#
		pref_con_GD_preference
...
metric_f_exp:
	...
	| {requirements.contains(":preferences")}? '(' 'is-violated' pref_name ')'	#
		metric_f_exp_is_violated;
